...
def generate_launch_description():
    ...
    map_file_edit = LaunchConfiguration('map_file_edit')
    arg_map_file_edit = DeclareLaunchArgument(
        'map_file_edit',
        default_value='warehouse_map_real_edit.yaml',
        description='Path to the map select'
    )

    # Obtener la ruta completa del archivo YAML del mapa
    map_file_path_edit = PythonExpression([
        "'",
        PathJoinSubstitution([
            FindPackageShare(package_localization),
            'maps',
    ...
    return LaunchDescription([ 
        ...
        Node(
            ...
            output='screen',
            parameters=[{'use_sim_time': False}, 
                        {'yaml_filename': map_file_path_edit},
                        {'topic_name' : "/map_edit"}
                        ]
        ),
